...
NetworkInterfaceInfo::get_interface_address()
{
    ...
    {
        close(m_fd);
        throw std::invalid_argument("get_ip_address exception");
    }
    ...
    {
        close(m_fd);
        std::cout << "Unable to read Netmask Address  for interface: " << m_interface_name << ", " << ex.what();
    }
    ...
    {
        close(m_fd);
        std::cout << "Unable to read MAC Address for interface: " << m_interface_name << ", " << ex.what();
    }
    m_socket_close();

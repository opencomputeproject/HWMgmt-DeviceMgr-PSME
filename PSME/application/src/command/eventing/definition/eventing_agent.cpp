...
/*added for ResourceAdded/ResourceRemoved/Alert/ResourceUpdated/StatusChange/ subscription*/
        ...
        event.set_event_origin_of_condition(request.get_event_origin_of_condition());
        event.set_event_oem(request.get_event_oem());
        event.set_event_message_id(request.get_event_message_id());

        log_debug(GET_LOGGER("rest"), "GET EVENT REQUEAST");
